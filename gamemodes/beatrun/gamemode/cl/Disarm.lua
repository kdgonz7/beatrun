...
local function Disarm_Render(self)
  ...
  owner:SetupBones()
  if owner.DisarmNoDraw then owner:SetNoDraw(true) end
  ...
  if cycle then self.bonelerpvalue = math.Clamp(self.bonelerpvalue + FrameTime() * (0.1 + self:GetCycle() * 2), 0, 1) end
end

function Disarm_CleanUp()
  local ply = LocalPlayer()
  if ply.DisarmVictim then ply.DisarmVictim:Remove() end
  if ply.DisarmVictimMDL then ply.DisarmVictimMDL:Remove() end
...
function Disarm_Think()
  ...
  if not ba or BodyAnimCycle >= 1 then Disarm_CleanUp() end
end
...
function Disarm_BlockMove(cmd)
  ...
  cmd:SetUpMove(0)
  cmd:SetButtons(0)
  if BodyAnimCycle > 0 and not lockang or not LocalPlayer():Alive() or LocalPlayer().DisarmForcedEnd < CurTime() then hook.Remove("CreateMove", "Disarm_BlockMove") end
end
...
net.Receive("DisarmStart", function() Disarm_Init(net.ReadEntity()) end)
hook.Add("CreateClientsideRagdoll", "Disarm_Ragdoll", function(ent, oldrag)
  if ent.InDisarm then
    ...
    oldrag:Remove()
    timer.Simple(30, function() if IsValid(rag) then rag:SetSaveValue("m_bFadingOut", true) end end)
    timer.Simple(0, function() Disarm_CleanUp() end)
  end
end)
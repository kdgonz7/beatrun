...
if CLIENT then
	...
	hook.Add("VManipPrePlayAnim", "LOCNoVManip", function()
		if LocalPlayer():UsingRH() or blinded then return false end end)
end
...
function SWEP:Think()
	...
	if self:GetHoldType() == "fist" and CurTime() > self:GetPunchReset() then self:SetHoldType("normal") end
	...
	local ismoving = vel:LengthSqr() > 100 and not ply:KeyDown(IN_BACK) and ply:IsSprinting() and not ply:Crouching() and not ply:KeyDown(IN_DUCK)
	...
	if spvel.z < -800 and ply:GetMoveType() ~= MOVETYPE_NOCLIP then
		...
		if CLIENT and fallct ~= CurTime() then
			local vel_origin = math.min(math.abs(spvel.z) / 2500, 1)
			local mult = 20
			fallang:SetUnpacked(2 * vel_origin * FrameTime() * mult, 1.25 * vel_origin * FrameTime() * mult, 1.5 * vel_origin * FrameTime() * mult)
			fallang:Add(ang)
			fallct = CurTime()
		end
		return
	...
	if ply:KeyPressed(IN_JUMP) and self:GetWasOnGround() and not ply:GetJumpTurn() then
		...
		if insidestep and viewmodel:GetCycle() <= 0.1 and GetConVar("Beatrun_QuakeJump"):GetBool() then
			if SERVER then ply:EmitSound("quakejump.mp3", 100, 100, 0.2) end
	...
	if onground then self:SetQuakeJumping(false) end
	...
	if self.punching and viewmodel:GetCycle() >= 1 then self:SendWeaponAnim(ACT_VM_DRAW) end
	if injump and (viewmodel:GetCycle() >= 1 or (ply:GetMantle() ~= 0 and ply:KeyDown(IN_JUMP)) or ply:GetWallrun() > 1) then self:SendWeaponAnim(ACT_VM_DRAW) end
	self:SetWeaponVelocity(Lerp(5 * FrameTime(), self:GetWeaponVelocity(), velocity))
	if not ismoving then self:SetWeaponVelocity(velocity) end
	if not self.punching and not insidestep then
		...
		elseif (curseq == 6 or curseq == 7) and (velocity < 50 or not ismoving or not onground) and curseq ~= 13 then
			self:SendWeaponAnim(ACT_VM_DRAW)
			if not onground and ply:GetSafetyRollKeyTime() <= CurTime() and ply:GetSafetyRollTime() <= CurTime() then ParkourEvent("fall", self:GetOwner()) end
		end
	end
	...
	if (curseq == 6 or curseq == 7) and ismoving then
		...
		if rate ~= ply:GetViewModel():GetPlaybackRate() then ply:GetViewModel():SetPlaybackRate(rate) end
	else
		ply:GetViewModel():SetPlaybackRate(1)
	end
...
function SWEP:PrimaryAttack()
	...
	if not ply:OnGround() or ply:GetSliding() or ply:GetGrappling() or ply:GetWallrun() ~= 0 or ply:GetJumpTurn() then
		return
	end
	local curseq = self:GetSequence()
	local infall = curseq == 19
	if infall then return end
	if CurTime() > self:GetPunchReset() then self:SetPunch(1) end
	...
	if tr_result.Hit then
		...
		if (SERVER and IsValid(ent)) and (not ply:IsPlayer() or (Course_Name == "" and not GetGlobalBool("GM_INFECTION"))) then
			...
			if ent:IsNPC() then ent:SetActivity(ACT_FLINCH_HEAD) end
		end

		if game.SinglePlayer() or (CLIENT and IsFirstTimePredicted()) then util.ScreenShake(Vector(0, 0, 0), 2.5, 10, 0.25, 0) end
	end

	self:SetPunch(punch + 1)
	if punch + 1 > 3 then self:SetPunch(1) end
end

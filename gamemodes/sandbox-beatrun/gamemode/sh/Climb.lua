...
elseif game.SinglePlayer() and CLIENT then
  net.Receive("Climb_SPFix", function()
    ...
    lockang = lock
    if oldlock ~= nil then lockang = oldlock end
    if neweyeang then LocalPlayer().OrigEyeAng = ang end
  end)
end
...
local function ClimbingThink(ply, mv, cmd)
  ...
  if ply:GetClimbing() == 2 or ply:GetClimbing() == 6 then
    ...
    local lerp = ply:GetClimbingTime()
    local lerprate = ClimbingTimes[ply:GetClimbing()]
    if lerp > 0.5 then lerprate = lerprate * 0.75 end
    local poslerp = LerpVector(lerp, ply:GetClimbingStart(), ply:GetClimbingEnd())
    mv:SetOrigin(poslerp)
...
local function ClimbingCheck(ply, mv, cmd)
  ...
  mins.z = 45
  tr.start = mv:GetOrigin()
  if ply:GetDive() then tr.start:Sub(Vector(0, 0, 48)) end
  ...
  if mv:KeyDown(IN_FORWARD) then
    ...
    if Vault4(ply, mv, eyeang, ply.mantletr, ply.mantlehull) then
      ...
      if ply:GetWallrun() == 1 then startpos.z = startpos.z + 17 end
      mv:SetOrigin(startpos)
      if Vault5(ply, mv, eyeang, ply.mantletr, ply.mantlehull) then
        if CLIENT then
          BodyAnimSetEase(origin)
  ...
  elseif game.SinglePlayer() then
    timer.Simple(0, function() ply:SendLua("BodyLimitX=80 BodyLimitY=170") end)
  end
...
hook.Add("SetupMove", "Climbing", function(ply, mv, cmd)
  ...
  if (not ply:GetCrouchJump() or ply:GetDive()) and not ply:GetJumpTurn() and (mv:KeyDown(IN_FORWARD) or mv:GetVelocity().z < -50 or ply:GetWallrun() == 1) and ply:GetClimbing() == 0 and ply:GetWallrun() ~= 4 and not ply:OnGround() and ply:GetMoveType() ~= MOVETYPE_NOCLIP and ply:GetMoveType() ~= MOVETYPE_LADDER then ClimbingCheck(ply, mv, cmd) end

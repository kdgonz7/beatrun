...
local function SafetyRollThink(ply, mv, cmd)
  ...
  if ply:OnGround() and not isRolling then lastGroundSpeed = mv:GetVelocity():Length() end
  local isRolling = CurTime() < ply:GetSafetyRollKeyTime()
  if ply:OnGround() and not isRolling then lastGroundSpeed = mv:GetVelocity():Length() end
  ...
  if CLIENT and ply:Alive() and IsValid(ply:GetActiveWeapon()) and CurTime() > ply:GetSafetyRollTime() then if weapons.IsBasedOn(ply:GetActiveWeapon():GetClass(), "mg_base") then RunConsoleCommand("mgbase_debug_vmrender", "1") end end
end
...
hook.Add("SetupMove", "EvadeRoll", function(ply, mv, cmd)
  if ply:GetJumpTurn() and ply:OnGround() and mv:KeyPressed(IN_BACK) then
    if CLIENT and ply:Alive() and IsValid(ply:GetActiveWeapon()) then if weapons.IsBasedOn(ply:GetActiveWeapon():GetClass(), "mg_base") then RunConsoleCommand("mgbase_debug_vmrender", "0") end end
  ...
  hook.Add("GetFallDamage", "SafetyRoll", function(ply, speed)
    ...
    if speed >= 800 and not ply:InOverdrive() and not ply:HasGodMode() then
      if speed < 800 and CurTime() < ply:GetSafetyRollKeyTime() and not ply:GetCrouchJump() and not ply:Crouching() then
        ...
        -- i think this is where you die
        local br_NormalDamage = GetConVar("Beatrun_NormalDamage"):GetInt()
        if br_NormalDamage == 1 then
          return 10
        else
          return 1000
        end

...
local function WallrunningThink(ply, mv, cmd)
  local wr = ply:GetWallrun()
  if wr ~= 0 and ply:OnGround() then ply:SetWallrunTime(0) end
  ...
  if wr == 1 and wrtimeremains then
    ...
    tr.output = trout
    util.TraceLine(tr)
    if not trout.Hit then ply:SetWallrunTime(0) end
  ...
  if wr >= 2 and wrtimeremains then
    ...
    if ply:GetWallrunElevated() then vecvel.z = 5 + math.Clamp(-(CurTime() - ply:GetWallrunTime() + 0.8) * 250, -400, 25) end
    if vecvel:Length() > 300 then vecvel:Mul(ply:GetOverdriveMult()) end
    mv:SetVelocity(vecvel)
    local eyeang = ply:EyeAngles()
    eyeang.x = 0
    if ply:GetVelocity():Length() <= 75 then ply:SetWallrunTime(0) end
    tr.start = ply:EyePos()
    ...
    if not trout.Hit and ply:GetWallrunTime() - CurTime() < hwrtime * 0.7 then
      ...
      if not trout.Hit then
        ...
        if not ply:GetWallrunDir():IsEqualTol(trout.HitNormal, 0.75) then ply:SetWallrunTime(0) end
    ...
    elseif ply:GetWallrunTime() - CurTime() < hwrtime * 0.7 then
      ...
      tr.output = trout
      util.TraceLine(tr)
      if trout.Hit and ply:GetWallrunDir():IsEqualTol(trout.HitNormal, 0.75) then ply:SetWallrunDir(trout.HitNormal) end
    end
  ...
  if ply:GetWallrunTime() < CurTime() or mv:GetVelocity():Length() < 10 then
    if ply.vwrturn == 0 then ply:SetQuickturn(false) end

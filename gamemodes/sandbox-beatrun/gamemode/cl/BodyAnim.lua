...
local armbones = {}
// Define arm bones for animation
armbones = {
  ...
  if transition and transitionlerp < 1 then
    ...
    for bone = 0, this:GetBoneCount() - 1 do
      if not armbones[BodyAnim:GetBoneName(bone)] then
        if not to[bone] then to[bone] = {{}, {}, {}} end
...
function StartBodyAnim(animtable)
  ...
  ply.OrigEyeAng.x = 0
  if VMLegs and VMLegs:IsActive() then VMLegs:Remove() end
  ...
  if usefullbody == 2 then
    ...
    for k, v in ipairs(playermodelbones) do
      ...
      if plybone then BodyAnimMDL:ManipulateBoneScale(plybone, vector_origin) end
    end

    if not ply:ShouldDrawLocalPlayer() then
      local head = BodyAnim:LookupBone("ValveBiped.Bip01_Head1")
      if head then BodyAnim:ManipulateBoneScale(head, vector_origin) end
    end
  ...
  if not showvm then ply:DrawViewModel(false) end
  BodyAnimCycle = 0
  DidDraw = false
  angclosenuff = false
...
function BodyAnimCalcView2(ply, pos, angles, fov)
  ...
  if has_tool_equipped then
    // note: this patches a stupid little buggy where
    // there's a thousand errors when the player dies w/ a physgun
    // t'was as simple as a IsValid check :)
    if ply:Health() >= 0 and IsValid(BodyAnim) then
      BodyAnim:SetNoDraw(true)
...
hook.Add("PostDrawOpaqueRenderables", "IgnoreZBodyAnim", function(depth, sky)
  if IsValid(BodyAnimMDL) then
    ...
    if ignorez then
      ...
      if not customarmdraw and IsValid(BodyAnimMDLarm) then BodyAnimMDLarm:DrawModel() end
      cam.IgnoreZ(false)
    end
  end
end)
...
hook.Add("CreateMove", "BodyAnim_Mouse", function(cmd)
  ...
  if not lockang and IsValid(BodyAnim) then
    ...
    if limitx ~= lastlimitx and pastlimitx or limity ~= lastlimity and pastlimity then BodyAnimLimitEase = true end
    if pastlimitx then
      local ang = nang

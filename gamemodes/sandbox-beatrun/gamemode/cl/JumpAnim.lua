...
-- The BodyAnim Cycle value is a number between 0 and 1 that controls the 
-- progress of an animation. 0 is the start of the animation, 1 is the end. 
-- Any value between 0 and 1 represents the progress of the animation. 
-- 
-- The numbers used in the addon are:
-- 0.0 - 0.15: Swing jump start
-- 0.15 - 1.0: Swing jump end
-- 0.0 - 0.35: Snatch scar start
-- 0.35 - 1.0: Snatch scar end
-- 0.0 - 0.75: Ladder climb start
-- 0.75 - 1.0: Ladder climb end
-- 0.0 - 0.35: Ladder enter start
-- 0.35 - 1.0: Ladder enter end
-- 0.0 - 1.0: Wall run vertical start
-- 0.0 - 0.75: Wall run vertical turn
-- 0.0 - 1.0: Vault over start
-- 0.0 - 1.0: Vault over high start
-- 0.0 - 1.0: Vault onto high start
-- 0.0 - 1.0: Falling uncontrolled
-- 0.0 - 1.0: Ladder exit start
-- 0.0 - 1.0: Crouch still
-- 0.0 - 1.0: Crouch forward
-- 0.0 - 1.0: Crouch backward
-- 0.0 - 1.0: Walk forward
-- 0.0 - 1.0: Run backward
-- 0.0 - 1.0: Stand idle
-- 0.0 - 1.0: Walk to stand left
local transitionchecks = {
	...
	jumpcoilend = function(ply)
		if ply:GetVelocity():LengthSqr() < 10 and BodyAnimCycle > 0.5 or BodyAnimCycle > 0.9 then return true end
	end,
	...
	if events[event] then
		...
		if event == "jump" or event == "jumpfar" or event:Left(11) == "jumpwallrun" and ply:GetWallrunDir():Dot(ply:EyeAngles():Forward()) < 0.75 then
			if event == "jumpfar" then
				...
				animtable.AnimString = "jumpstill"
			end
